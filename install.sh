...
echo_latest_version() {
  if [ "${EDGE-}" ]; then
    version="$(curl -fsSL https://api.github.com/repos/originalbluef3x/code-server-enhanced/releases | awk 'match($0,/.*"html_url": "(.*\/releases\/tag\/.*)".*/)' | head -n 1 | awk -F '"' '{print $4}')"
  else
    # https://gist.github.com/lukechilds/a83e1d7127b78fef38c2914c4ececc3c#gistcomment-2758860
    version="$(curl -fsSLI -o /dev/null -w "%{url_effective}" https://github.com/originalbluef3x/code-server-enhanced/releases/latest)"
  fi
  version="${version#https://github.com/originalbluef3x/code-server-enhanced/releases/tag/}"
  version="${version#v}"
  echo "$version"
}
...
install_deb() {
  ...
  fetch "https://github.com/originalbluef3x/code-server-enhanced/releases/download/v$VERSION/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "https://github.com/originalbluef3x/code-server-enhanced/releases/download/v$VERSION/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "https://github.com/originalbluef3x/code-server-enhanced/releases/download/v$VERSION/code-server-$VERSION-$OS-$ARCH.tar.gz" \
  ...
  "$sh_c" ln -sfn "$STANDALONE_INSTALL_PREFIX/lib/code-server-$VERSION" "$STANDALONE_INSTALL_PREFIX/lib/code-server-current"
  echo_standalone_postinstall
}
